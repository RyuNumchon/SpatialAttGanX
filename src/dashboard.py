...
if st.button('Generate'):
    ...
    st.image(f'/outputs/disentangled_features/{num_features}/output_{random_idx}.png')
    st.text("Original Image from GAN")
    input_path = f'/outputs/disentangled_features/{num_features}/input_{random_idx}.png'
    st.text("GAN with Multiple Distangled features")
    st.text(f"Total: {num_features} disentangled features")
    output_path = f'/outputs/disentangled_features/{num_features}/output_{random_idx}.png'
    st.image(input_path)
